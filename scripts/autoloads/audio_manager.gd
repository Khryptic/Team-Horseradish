...
var random_sound = RandomNumberGenerator.new()
...
func create_2d_audio_at_location(location: Vector2, type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			new_2D_audio.stream = sound_effect.sound_effect[random_sound.randi_range(0, sound_effect.sound_effect.size() - 1)]
...
func create_audio(type: SoundEffect.SOUND_EFFECT_TYPE) -> void:
	if sound_effect_dict.has(type):
		...
		if sound_effect.has_open_limit():
			...
			add_child(new_audio)
			new_audio.stream = sound_effect.sound_effect[random_sound.randi_range(0, sound_effect.sound_effect.size() - 1)]
